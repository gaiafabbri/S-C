...
Gen_files(){
    while true; do
        echo "Choose whether to download or generate the file; if you download it, only 10 000 events are present in the file"
        ...
        case $choice in
            ...
            [Dd]* )
                ...
                # Check if the downloaded file is a ROOT file
                echo "Sometimes problems with the download can occur: please check the file is in the correct format"
                read -p "Is it a root file (and not html)? (y/n): " is_root
                if [ "$is_root" = "y" ]; then
                    echo "Continuing..."
                    break
                elif [ "$is_root" = "n" ]; then
