...
def accuracy(outputs, targets):
    # Converte le probabilità in predizioni di classe (0 o 1)
    predicted = outputs > 0.5
    # Confronta le predizioni con le etichette target
...
def test_eval(model, test_loader, criterion):
    ...
    print('Test accuracy:', test_acc) #serve questa funzione? cosa fa?
...
def torch_eval(trained_model, test_loader):
    ...
    y_pred_classes = (y_pred > 0.5).astype(int)  # Arrotonda le probabilità alle classi binarie

    # Estrai i valori effettivi delle etichette dal DataLoader
    true_labels = []
    for _, labels in test_loader:
        true_labels.extend(labels.numpy())  # Converti tensori in numpy array e aggiungi alla lista
    true_labels = np.array(true_labels)  # Converti la lista in un array NumPy
    ...
    return true_labels, y_pred, y_pred_classes
